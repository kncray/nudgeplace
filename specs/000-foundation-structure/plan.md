...
- 경계 적합성 픽스처 앱은 도메인 모델·primitive **정의 0개**(SC-003 — 기계 검증),
  cross-app 내부 import **0건**(SC-005). Spec 002부터 커널 primitive의 **사용**은
  허용되지만 **정의** 금지는 유지된다(정의/사용 분리).
...
Two-tier gate per Constitution governance: (1) CORE gates — VII, IV, VIII, I, V, VI — always evaluated; a core gate that genuinely does not apply remains visible and is recorded as N/A with rationale. (2) CONDITIONAL gates use the Constitution's single-source trigger matrix. **이 스펙은 도메인·금액·이벤트·외부연동·자원선점·벤더·레거시 재사용이 전무하므로 조건부 게이트가 하나도 트리거되지 않는다.** 아래 표는 코어 6개만 평가한다(기계적 전수 재평가 방지).
...
**조건부 게이트**: 트리거 없음(IX·X·XI·XII·XIII·XIV·XV·XVI·III·XVII 모두 해당 도메인/
...
- **Review evidence**: 조건부 통과(HIGH 1건 포함 지적 9건 — 반영 8건·게이트 확인 1건) —
  ...
  분리로 정정(정의 금지 유지, Spec 002부터 사용 허용), (8) spec/checklist 승인 상태
