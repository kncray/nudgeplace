...
**Phase**: 0 — Foundation | **Spec Sequence**: 000 (Foundation Step 1 / 3)

**Created**: 2026-07-08

**Status**: Approved <!-- Draft → In Review → Approved -->

**Input**: 로드맵 Phase 0(Foundation)을 학습 단위로 3개 스펙으로 분할한 첫 번째
스펙. 이 스펙은 **프로젝트 구조와 기계적 도메인 경계, 그리고 문서(ADR·소유 규칙)**에
한정한다. 공유 커널 primitive(Money·이벤트·멱등·감사·역할)는 Spec 001, 관통 walking
skeleton은 Spec 002, ACL 골격은 P6로 이관(슬림화)한다.
...
Foundation(Phase 0)은 이 원칙에 따라 세 개의 학습 단계(Step)로 나눈다:

- **Step 1 (Spec 000, 이 스펙) — 구조·경계**: 모듈러 모놀리스 골격과 기계적 도메인 경계, 문서 소유 규칙(ADR).
- **Step 2 (Spec 001) — 공유 커널 primitive**: Money·도메인 이벤트 계약·멱등키·감사·행위자 역할.
- **Step 3 (Spec 002) — 관통 walking skeleton**: 위 primitive들이 실제로 함께 배선됨을 관통 증명.

**왜 구조·경계가 먼저인가**: 도메인 경계가 기계적으로 강제되지 않으면 "모듈화된"
모놀리스가 강결합 모놀리스로 퇴화한다(원칙 VIII). 이 규율은 이후 모든 Phase(그리고
Spec 001·Spec 002 자신)가 그 위에 놓이는 토대이므로, 어떤 primitive나 도메인보다 먼저
...
- **경계 적합성 픽스처 (Boundary Conformance Fixture)**: 경계 강제가 실제로
  ...
  갖는다. (Spec 002의 walking skeleton이 이 앱들을 확장해 primitive를 관통시킨다.)
...
- **Scope boundaries** (Governance / range constraint): **명시적 범위 밖** —
  ...
  (2) primitive를 관통 검증하는 walking skeleton → **Spec 002**;
...
- **경계 적합성 픽스처의 수명**: 픽스처 앱은 도메인·primitive를 담지 않는 아키텍처
  적합성 스모크 테스트로 유지하며, **Spec 002의 walking skeleton이 이를 확장**해
  primitive를 관통시킨다(그 시점에도 실제 도메인 모델은 넣지 않는다).
...
- **후속 Foundation 스펙**: Spec 001(공유 커널 primitive — Money·이벤트·멱등·감사·
  역할)와 Spec 002(관통 walking skeleton — 모든 primitive를 실제 도메인 없이 E2E로
  관통)는 이 스펙 완료 후 각자 `/speckit-specify`로 작성한다.
